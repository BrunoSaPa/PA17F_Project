...
public static class Utilidades
{
	...
	public static byte[] ObtenerFechaActualEnBytes()
	{
	    byte[] bytes = BitConverter.GetBytes(DateTime.Now.Ticks);
	    return bytes;
	}

	public static DateTime ObtenerFechaDesdeBytes(byte[] fechaBytes)
	{
	    if (fechaBytes == null || fechaBytes.Length != 8)
	    {
	        throw new ArgumentException("El arreglo de bytes de fecha no es válido.");
	    }
	
	    long ticks = BitConverter.ToInt64(fechaBytes, 0);
